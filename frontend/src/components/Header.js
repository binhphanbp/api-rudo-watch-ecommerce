...
// 1. Lấy thông tin User từ LocalStorage (Do file auth.js lưu vào khi login thành công)
const user = JSON.parse(localStorage.getItem('user'));
const isLoggedIn = !!user; // Nếu có user -> true, ngược lại -> false

// Avatar: Nếu user có avatar thì dùng, không thì dùng ảnh tạo tự động theo tên
...
// Hàm xử lý Đăng xuất (Gắn vào window để gọi được từ HTML string)
window.handleLogout = () => {
  localStorage.removeItem('token');
  localStorage.removeItem('user');
  // Reload lại trang để Header cập nhật lại trạng thái chưa đăng nhập
...
export function Header() {
  ...
  return `
    <header class="w-full bg-[#0A2A45] text-white sticky top-0 z-50 border-b border-white/10 shadow-lg relative group/header">
        ...
        <div id="search-overlay" class="absolute inset-0 bg-white text-slate-900 z-[60] transform -translate-y-full transition-transform duration-300 flex items-center px-8 shadow-xl">
        ...
        <div class="absolute top-0 left-0 w-full h-[3px] bg-transparent z-10">
            <div id="scroll-progress" class="h-full bg-gradient-to-r from-blue-400 to-cyan-300 w-0 transition-all duration-150 ease-out"></div>
        </div>
